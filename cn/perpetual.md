...
MCDEX永续合约是一种类似于期货合约的衍生品，但没有到期日期。MCDEX永续合约使用了我们在以太坊上的[Mai Protocol V2](https://github.com/mcdexio/mai-protocol-v2) 智能合约。

目前，MCDEX永续合约有两个交易页面，对应两种交易方式：

- **订单簿**：类似于中心化交易所的永续合约。
- [**AMM (自动做市商)**](#automated-market-maker)：完全去中心化的交易界面，简洁直观的用户体验。

自动做市商始终在线，并以预设的定价公式作为交易对手提供交易服务。当然，交易者还可以选择订单簿进行交易，以获得更好的流动性，更小的滑点以及类似于中心化交易所的交易体验。

MCDEX永续合约通过[实时资金费用机制](#funding-rate)以将永续合约的价格平滑锚定到[指数价格](#index-oracle)。

如果永续合约的交易价格高于指数价格，则多头仓位持有人向空头仓位持有人支付资金费用。通过这个方式增加了多头头寸持有者的持有成本，导致他们最终进行了卖出操作，从而将永续合约价格压低到指数价格。
...
资金费用每秒钟自动计算一次，并实现到已实现盈亏中（也是您可用交易余额的一部分）。您可以随时从保证金账户中提取已实现的盈亏。
...
|初始保证金|10%|
|维持保证金|7.5%|
|标记价格|交易时间内该永续合约的估计价格。它可以（暂时）偏离实际的永续合约市场价格，以防止有人操纵交易。标记价格的计算方法是：指数价格+公允价格-指数价格的600秒EMA。永续合约公允价格是AMM（自动做市商）的中间价格。
|交割/到期时间|	没有交割/到期时间|
|交割方式| ETH现金交割|
|合约类型| [反向合约](#vanilla-amp-inverse-contract) |

## 交易示例

每位交易者有一个[独立保证金](#isolated-margin) 账户。一个保证金账户包含 `保证金余额` 和 `仓位`。用户可以存钱到 `保证金账户` ，实时结算的PNL也会实现到 `保证金账户`中。 PNL的计算方法与合同类型有关，我将在以下示例中进行解释。

### ETH-PERP

ETH-PERP是一种[反向合约](#vanilla-amp-inverse-contract)。它的保证金是ETH，合约价值1美元。
...
&ast;&ast; 此示例假设[标记价格](#mark-price) = [指数价格](#index-oracle)
...
要了解如何计算标记价格，我们得先确定“公允价格”。为了使标记价格和资金利率独立于任何链下设施，MCDEX使用链上AMM（自动做市商）的“中间价格”作为“公允价格”。更多详细信息请在相应的章节中查看。
...
2. 与AMM交易，相当于做空，价格为x / y。交易金额计算如下：
...
在添加流动性时，流动性提供者将抵押代币和多头头寸都添加到池中：1.提供者将抵押代币发送到池中；2.同时，提供者将与流动性池进行交易，方向为卖出，这将增加流动性池的多头头寸。添加后，提供者在其保证金帐户中有一个空头头寸，并且池中多头头寸的份额为零，净头寸应该为零。但是，当其他交易者跟池子进行交易时，池的头寸大小会发生变化。流动性提供者的净头寸规模不为零，这是风险敞口。流动性提供者会获得交易费作为奖励。

在 [此处](en/perpetual-tech#add-liquidity-to-amm)查看更多示例。


### 提取流动性
共享令牌持有者可以从池中消除流动性并赎回共享令牌。让我们为交易者在资金池中的份额（百分比）。为了消除流动性，交易者调用智能合约来执行以下操作：
1. 与AMM进行交易，交易价格为 ```x/y```。交易金额为 ```c = y∙s```
...
为了与订单簿进行交易，交易者签名他们的订单并将订单发送到订单簿服务器。订单簿服务器匹配订单簿中的订单，并将匹配结果发送到区块链上的智能合约。智能合约首先验证订单的签名和匹配结果。如果通过验证，则根据匹配结果进行交易。

### 目标杠杆

无论与AMM还是与订单簿进行交易，头寸的有效杠杆始终为 ```头寸价值 / 保证金余额``` ，而未结头寸的最大杠杆为 ```1 / 初始保证金```

然而，交易者可以在与订单簿进行交易时设置“目标杠杆”。订单簿引擎使用目标杠杆来计算头寸保证金和订单保证金，并自动取消可能将头寸杠杆推高目标杠杆的订单。

须知，目标杠杆率只是链下订单簿服务器中的设置。即使交易者设置了低目标杠杆，链上头寸的有效杠杆也可能高于或低于目标杠杆。例如，交易者从保证金账户中提取抵押代币，从而导致有效杠杆的增加。另一个例子，头寸的利润不断增加，这导致保证金余额增加，有效杠杆降低。

### 头寸和订单保证金
订单簿服务器计算用于成交新订单的所需保证金，如下所示：（普通合同）
...
```订单保证金``` 是订单簿服务器为正在进行的订单预留的保证金。已完成的订单将无需订单保证金。对于已成交并成为头寸的订单，将计算订单保证金：
...
经纪人是帮助交易者完成交易的角色。这是订单的普通ETH地址。在每个订单的签名中设置经纪人。要收取交易费，经纪人必须在订单数据结构中分配正的制造商/承办商费率。如果给定负交易费，则经纪人将向交易员支付作出/接受定单的费用。

## AMM与订单簿之间的关系

请注意，我们的永续合约可以在没有订单簿的情况下使用。如果订单簿由于某种原因而关闭，则永续合约系统仍然可用，因为资金费率是右AMM提供。链下订单加强了整个系统。随着套利者在订单簿和AMM之间移动流动性，AMM具有更好的流动性和价格。该系统的设计旨在通过丰富的链上设计来让用户满意。

## 指数预言机

指数价格源来自Chainlink。MCDEX 永续合约依赖于指数预言机的公平性和正确性。一些加密项目致力于提供去中心化的预言机服务。虽然并不完美，但我们认为在早期将Chainlink用作预言机是一个不错的选择。

## 自动清算

Mai2永续智能合约按以下方式计算保证金帐户（普通合约）：
...
任何人都可以在未经帐户所有者许可的情况下清算不安全保证金帐户。清算人以标记价格接管不安全头寸，账户向清算人和保险基金支付清算罚款。由于清算后将不安全头寸转移到清算人，清算人必须有足够的保证金余额以保持头寸安全。

如果清算时的标价比破产价差，则标价和破产价之差就是清算损失。损失首先由保险基金弥补。如果资金不足以弥补损失，智能合约将使损失社会化。在这种情况下，所有对手持仓者根据其仓位大小分担损失。

## 全局结算

由于区块链基础设施的效率低下，清算机制受到限制。在极端情况下，例如剧烈的价格变化，错误的指数价格输入等，将造成大量的社会损失。在此期间，MCDEX将评估情况，并在必要时确定结算价格，并将合同纳入全局结算情况。当进入这种情况时，所有交易将被中止。没有足够保证金的账户将以结算价清算。并且系统损失将被社会化。清算结束后，用户可以按结算价结算其头寸，并提取所有剩余的保证金余额。
